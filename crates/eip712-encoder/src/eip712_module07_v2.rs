...
pub mod tests_eip712_module07 {
    ...
    #[tokio::test]
    async fn test_eip712_by_ethers_m07_command_sponsor()  {
        /*
...
pub mod custom_712_module07 {
    ...
    ///
    /// keccak256 of "EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)""
    ///
    ...
    */
    ...
    async fn test_encoded_and_sign_recover() {
        ...
        let compact_sig = compact(&signature);
    ...
    /*
    // cargo test --package evm-related --lib -- eip712_module07_v2::custom_712_module07::test_encoded_and_sign_recover_2 --exact --show-output
    #[tokio::test]
    async fn test_encoded_and_sign_recover_2() {
        // Setup
        let domain = DebugEIP712Domain {
            name: "ZapGeneralizedIO".to_string(),
            version: "1".to_string(),
            chain_id: 9889,
            verifying_contract: Address::from_str("0x0000000000000000000000000000000000000001").unwrap(),
        };

        let zerob256: [u8; 32] = [0x00; 32];
        let zero_value = H256::from_slice(zerob256.as_ref());

        let asset1_in = H256::from_slice(convert_hex_str_to_h256(ASSET_IN_ID).as_ref());

        let utxo1 = H256::from_slice(convert_hex_str_to_h256(UTXO_1).as_ref());
        let utxo2 = H256::from_slice(convert_hex_str_to_h256(UTXO_2).as_ref());


        let amount_in_1 = U256::from(1_000_000_000u64);
        let amount_in_2 = U256::from(520_000_000u64);
        let amount_zero = U256::from(0u64);

        let asset_out = H256::from_slice(convert_hex_str_to_h256(ASSET_OUT_ID).as_ref());

        let amount_out = U256::from(2_000_000_000u64);

        let tollerance = U256::from(250u64);


        let tx = GenIO {
            inputassets: [asset1_in, asset1_in, zero_value, zero_value, zero_value],
            inpututxoids: [utxo1, utxo2, zero_value, zero_value, zero_value],
            inputamounts: [amount_in_2, amount_in_1, amount_zero, amount_zero, amount_zero],
        ...
        // Convert to 32 bytes
        let mut bytes1 = [0u8; 32];
        tx.inputamounts[0].to_big_endian(&mut bytes1);
        println!("amountin U256 value: {}", tx.inputamounts[0]);
        println!("amountin 32-byte representation (hex): 0x{}", hex::encode(bytes1));
        println!(" ");
        let mut bytes1_1 = [0u8; 32];
        tx.inputamounts[1].to_big_endian(&mut bytes1_1);
        println!("amountin U256 value: {}", tx.inputamounts[0]);
        println!("amountin 32-byte representation (hex): 0x{}", hex::encode(bytes1_1));


        println!(" ");
        let mut bytes2 = [0u8; 32];
        tx.outputamount.to_big_endian(&mut bytes2);
        println!("amountout U256 value: {}", tx.outputamount);
        println!("amountout 32-byte representation (hex): 0x{}", hex::encode(bytes2));
        let mut bytes3 = [0u8; 32];
        tx.tolerance.to_big_endian(&mut bytes2);
        println!("tolerance U256 value: {}", tx.tolerance);
        println!("tolerance 32-byte representation (hex): 0x{}", hex::encode(bytes3));
        ...
        // Create a wallet from a private key
        // evm address: 0x222227f7e08997ee7457a0a772e417ca5462906d
        // private key: 0b4895d01ae5996f3fafd5e008f7802b3f8ccd3440ec9076d04c3d9b00593b5b

        let private_key = "0x0b4895d01ae5996f3fafd5e008f7802b3f8ccd3440ec9076d04c3d9b00593b5b";
        let wallet_from_key = LocalWallet::from_str(private_key).unwrap();
        println!("Wallet Address: {}", hex::encode(wallet_from_key.address()));
